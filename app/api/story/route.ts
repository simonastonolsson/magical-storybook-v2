...
export async function POST(req: Request) {
  try {
    ...
    // LÅS SIGNATUR-OUTFITEN (Den absoluta hemligheten för en röd tråd!):
    // Vi tvingar karaktärerna att alltid bära exakt samma ikoniska kläder genom hela boken.
    ...
    const fullPrompt = `You are an expert comic book director. Create a dynamic comic book script based on the user's idea: "${prompt}".
      ...
      DIRECTOR RULES FOR WRITING (CRITICAL FOR NARRATIVE FLOW & RESEMBLANCE CONSISTENCY):
      ...
      4. CAMERA ANGLES FOR PERFECT LIKENESS (STRICT CONSISTENCY RULE):
         ...
         - Always write prompts specifying a "three-quarter face view (trekvartsprofil)" or "waist-up three-quarter shot" whenever depicting ${trigger}'s face. This is the optimal angle for facial resemblance.
      ...
      2. CAMERA FRAMING RULE: 
         - Always use explicit out-zooming descriptions: "wide angle", "shot from a distance", "establishing shot", "action shot", "three-quarter body shot showing the characters engaged in...", "three-quarter face view (trekvartsprofil)".
