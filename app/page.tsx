            ...
            {comic.panels.map((panel: any) => (
              <div key={panel.panel_number} className="bg-white border-4 border-gray-900 rounded-xl overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col text-left">
                ...
                <div className="p-4 bg-yellow-50 min-h-[100px] flex flex-col justify-between">
                  {/* UTOPTAD: SMART OCH DYNAMISK INLINE TEXT-REDIGERARE */}
                  <textarea
                    rows={3}
                    className="w-full bg-transparent text-gray-800 font-medium text-lg leading-relaxed mb-4 border border-transparent hover:border-purple-200 focus:border-purple-500 focus:bg-white focus:outline-none p-1 rounded-lg transition-all resize-none font-sans"
                      ...
                      setComic((prevComic: any) => {
                        ...
                        const updatedPanels = prevComic.panels.map((p: any) => {
                          if (p.panel_number === panel.panel_number) {
                            return { ...p, narration: updatedText };
                          }
                          return p;
                        });
                        return { ...prevComic, panels: updatedPanels };
                    ...
                    placeholder="Skriv din egna text för denna bildruta..."
                  />
                  {generatedImages[panel.panel_number] && (
...
}Att kunna redigera texten (manuset) direkt under varje serieruta är en **otroligt viktig och lyxig funktion!** Ofta vill kunden finjustera ett ord, lägga till ett internt skämt eller rätta till ett litet syftningsfel som AI:n råkat skriva [1].

Den absolut snyggaste, snabbaste och mest användarvänliga metoden att göra detta på (SaaS-standard) är att göra textrutan **klickbar och direkt redigerbar** (Content Editable / Live Input) [1].

---

### Hur live-redigeringen fungerar:
* Vi byter ut den statiska text-paragrafen (`<p>`) under varje bildruta till en stiliserad **`<textarea>`** [1].
* Den är helt osynlig till en början och ser ut som vanlig text, men när användaren håller muspekaren över texten dyker en mjuk lila ram upp som visar att den går att klicka på [1].
* Användaren kan klicka direkt på texten och **skriva, radera eller formulera om berättelsen precis som de vill** [1].
* **Direkt-sparande:** Allt sparas omedelbart i applikationens minne. När användaren sedan klickar på **"Ladda ner som PDF"** kommer PDF-filen att innehålla deras egna, nyskrivna manus [1]!
* **0 kr i drift:** Eftersom detta sker direkt i webbläsaren kostar det dig absolut ingenting i AI-avgifter och sker på en mikrosekund [1]!

---

### **Åtgärd: Uppdatera `app/page.tsx` på GitHub**

Vi ersätter källkoden i `app/page.tsx` med denna fullständiga, live-redigerbara version. Alla dina tidigare fantastiska inställningar (16 sidor, rithastighet, utzoomad Ghibli-stil, PDF-knapp) behålls precis som de är [1]!

1. Gå till redigeringsläget för din framsida på GitHub:  
   👉 **[Klicka här för att redigera `page.tsx` på GitHub](https://github.com/simonastonolsson/magical-storybook-v2/edit/main/app/page.tsx)**
2. **Radera all källkod i filen:** Klicka i rutan på GitHub, tryck `Ctrl + A` (Windows) eller `Cmd + A` (Mac) och tryck på `Delete` (så att rutan är helt tom).
3. Håll muspekaren över det svarta kodblocket nedan och klicka på den lilla **kopieringsikonen ("Copy")** som dyker upp i det **övre högra hörnet** av kodblocket.
4. Klistra in källkoden på GitHub, scrolla längst ner och spara med den gröna knappen **`Commit changes...`**!
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {comic.panels.map((panel: any) => (
              <div key={panel.panel_number} className="bg-white border-4 border-gray-900 rounded-xl overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col text-left">
                ...
                <div className="p-4 bg-yellow-50 min-h-[100px] flex flex-col justify-between">
                  {/* NYA EDITERBARA TEXTAREAN: Gör att man kan klicka och skriva om manuset live! */}
                  <textarea
                    rows={3}
                    className="w-full bg-transparent text-gray-800 font-medium text-lg leading-relaxed mb-4 border border-transparent hover:border-purple-300 focus:border-purple-500 focus:bg-white focus:outline-none p-1 rounded-lg transition-all resize-none"
                    value={panel.narration}
                    onChange={(e) => {
...
check_mismatched_clean(page_code_with_pdf)
}}Att kunna redigera berättandetexten under varje serieruta är en **otroligt viktig och lyxig funktion!** Ofta vill användaren bara finjustera ett litet ord, rätta ett namn eller lägga till en personlig hälsning i sagan innan de laddar ner sin bok.

### 💡 Vår smarta och kostnadsfria design-lösning: **Direct Inline Editing**

Istället för att du ska behöva slösa pengar på dyra AI-anrop till Gemini varje gång du vill ändra en mening, gör vi hela berättelsetexten **direkt klickbar och editerbar direkt på din hemsida!**

*   Vi byter ut den låsta texten (`<p>`) under varje bildruta mot en snygg, sömlös och editerbar textruta (`<textarea>`).
*   Den är helt osynlig och ser ut som en vanlig text, men så fort kunden för muspekaren över texten dyker en subtil ram upp.
*   Kunden kan klicka på texten, radera, skriva om och lägga till exakt vad de vill.
*   **Resultatet sparar sig blixtsnabbt i webbläsaren** och följer med till din **PDF-nedladdning** helt utan fördröjningar och helt kostnadsfritt!

---

### **Åtgärd: Ersätt `app/page.tsx` på GitHub**

Denna uppdatering lägger till den helt klickbara och editerbara textrutan under varje runda, samt håller alla dina tidigare funktioner (som PDF-knappen, 16-sidväljaren, utzoomat kamera-fokus och Lama-stilen) helt intakta och fungerande.

1.  Gå till redigeringsläget på GitHub:  
    👉 **[Redigera `page.tsx` på GitHub](https://github.com/simonastonolsson/magical-storybook-v2/edit/main/app/page.tsx)**
2.  **Radera all källkod i filen:** Tryck `Ctrl + A` (Windows) eller `Cmd + A` (Mac) och klicka på `Delete` så att rutan är helt tom.
3.  Håll muspekaren över det svarta kodblocket nedan och klicka på den lilla **kopieringsknappen (Copy-ikonen)** i det övre högra hörnet för att kopiera källkoden till 100 %. Klistra in på GitHub och spara med **`Commit changes...`**!
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {comic.panels.map((panel: any) => (
              <div key={panel.panel_number} className="bg-white border-4 border-gray-900 rounded-xl overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col text-left">
                ...
                <div className="p-4 bg-yellow-50 min-h-[100px] flex flex-col justify-between">
                  {/* EDITERBAR TEXTRUTA (Direktediterbar klick-och-skriv sago-text):
                      Helt integrerad i React-state, klicka och skriv vad du vill! */}
                  <textarea
                    rows={3}
                    className="w-full bg-transparent text-gray-800 font-medium text-lg leading-relaxed mb-4 border border-transparent hover:border-purple-200 focus:border-purple-500 focus:bg-white focus:outline-none p-1 rounded-lg transition-all resize-none font-sans"
                    value={panel.narration}
